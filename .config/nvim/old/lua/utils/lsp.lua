...
--     local root = assert(LazyVim.root.realpath(LazyVim.root.get({
--         normalize = true
--     })))
...
function M.words.setup(opts)
    ...
    M.on_supports_method("textDocument/documentHighlight", function(_, buf)
        vim.api.nvim_create_autocmd({"CursorHold", "CursorHoldI", "CursorMoved", "CursorMovedI"}, {
            group = vim.api.nvim_create_augroup("lsp_word_" .. buf, {
                clear = true
            }),
            buffer = buf,
...
function M.words.get()
    ...
    for _, extmark in ipairs(vim.api.nvim_buf_get_extmarks(0, M.words.ns, 0, -1, {
        details = true
    })) do
        local w = {
