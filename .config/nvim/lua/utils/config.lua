...
function M.load_plugin(plugin_name, opts, vim_plugin, setup_callback)
  ...
  if ok and vim_plugin then
    ...
    elseif type(user_config) == "function" then
      ...
      vim.notify(
        string.format("<%s> is not a typical Lua plugin, please return a function with\n" ..
          "the corresponding options defined instead (usually via `vim.g.*`)", plugin_name),
        vim.log.levels.ERROR,
...
We received a `%s` for plugin <%s>.]], type(user_config), plugin_name),
          vim.log.levels.ERROR,
          { title = "[utils] Runtime Error (User Config)" }
        )
      end
