...
local function check_executables()
    local checks = {"git", "rg", {"fd", "fdfind"}, "lazygit", "nvr", -- formatters / linters
    "stylua", "selene", "shfmt", "black", "jq",  -- platform helpers
...
function M.check_blink_cmp()
    ...
    if has_cargo then ok("cargo detected") else warn("missing cargo (Rust build unavailable)") end
    if has_rustup then ok("rustup detected") else warn("missing rustup (nightly management unavailable)") end
