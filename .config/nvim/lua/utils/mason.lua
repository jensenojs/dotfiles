...
M.groups = {
  ...
  go = {
    ...
    tools = { "golangci-lint", "gofumpt", "gotests", "goimports", "gomodifytags", "impl", "go-debug-adapter", "delve" },
  },
  python = {
    servers = { "pyright" },
...
function M.servers()
  ...
  for _, g in pairs(M.groups) do
    if g.servers then
      for _, s in ipairs(g.servers) do table.insert(acc, s) end
    end
  end
  return flatten(acc)
end
...
function M.tools()
  ...
  for _, g in pairs(M.groups) do
    if g.tools then
      for _, t in ipairs(g.tools) do table.insert(acc, t) end
    end
  end
  return flatten(acc)
end
