...
vim.api.nvim_create_autocmd("LspAttach", {
    ...
    callback = function(event)
        ...
        local lsp_keymaps = {
            ...
            end):with_buffer(bufnr):with_noremap():with_silent():with_desc(
                "LSP: (悬停时)显示文档、类型、定义位置等静态信息"),
            ["in|<C-k>"] = map_callback(function()
                if if_support(vim.lsp.protocol.Methods.textDocument_signatureHelp) then
                    vim.lsp.buf.signature_help()
                end
            end):with_buffer(bufnr):with_noremap():with_silent():with_desc("LSP: Signature Help"),

            -- 
            ["n|<leader>lwa"] = map_callback(vim.lsp.buf.add_workspace_folder):with_buffer(bufnr):with_noremap()
                :with_silent():with_desc("LSP: (添加)工作区文件夹"),
            ["n|<leader>lwd"] = map_callback(vim.lsp.buf.remove_workspace_folder):with_buffer(bufnr):with_noremap()
                :with_silent():with_desc("LSP: (删除)工作区文件夹"),
            ["n|<leader>lwl"] = map_callback(vim.lsp.buf.list_workspace_folders):with_buffer(bufnr):with_noremap()
                :with_silent():with_desc("LSP: (列出)工作区文件夹"),

            ["n|<leader><f2>"] = map_callback(function()
                if if_support(vim.lsp.protocol.Methods.textDocument_rename) then
                    vim.lsp.buf.rename()
                end
            end):with_buffer(bufnr):with_noremap():with_silent():with_desc("LSP: (重命名)Symbol"),

            ["n|<leader>lca"] = map_callback(function()
                if if_support(vim.lsp.protocol.Methods.textDocument_codeAction) then
                    vim.lsp.buf.code_action()
                end
            end):with_buffer(bufnr):with_noremap():with_silent():with_desc("LSP: Code Action"),
            ...
            end):with_buffer(bufnr):with_noremap():with_silent():with_desc("LSP: (查找)被谁调用"),

            ["n|<leader>lco"] = map_callback(function()
                if client.server_capabilities and client.server_capabilities.callHierarchyProvider then
                    vim.lsp.buf.outgoing_calls()
                end
            end):with_buffer(bufnr):with_noremap():with_silent():with_desc("LSP: (查找)我调用谁"),
            ...
            end):with_buffer(bufnr):with_noremap():with_silent():with_desc("LSP: (查找)子类型"),
            ...
            end):with_buffer(bufnr):with_noremap():with_silent():with_desc("LSP: (查找)父类型"),
            ...
            end):with_buffer(bufnr):with_noremap():with_silent():with_desc("LSP: (打开)诊断信息"),
            ...
            end):with_buffer(bufnr):with_noremap():with_silent():with_desc("LSP: (打开)当前文件的诊断列表"),

            ["n|<leader>lD"] = map_callback(function()
                vim.diagnostic.setqflist({
                    open = true
                })
            end):with_buffer(bufnr):with_noremap():with_silent():with_desc("LSP: (打开)工作区诊断列表"),
            ...
            ["n|[d"] = map_callback(vim.diagnostic.goto_prev):with_buffer(bufnr):with_noremap():with_silent():with_desc(
                "LSP: (跳转到)上一个诊断(快捷)"),
            ["n|]d"] = map_callback(vim.diagnostic.goto_next):with_buffer(bufnr):with_noremap():with_silent():with_desc(
                "LSP: (跳转到)下一个诊断(快捷)"),
            ...
            end):with_buffer(bufnr):with_expr():with_noremap():with_silent():with_desc(
                "LSP: (自动补全)选择下一个"),
        ...
        -- 光标处文档高亮
        if if_support(vim.lsp.protocol.Methods.textDocument_documentHighlight) and vim.bo[bufnr].filetype ~= 'bigfile' then
