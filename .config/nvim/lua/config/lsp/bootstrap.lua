-- 作用: LSP 运行时初始化
-- 职责:
--   1. 注册所有 LSP 服务器配置 (vim.lsp.config)
--   2. 设置全局 LSP handlers 和 diagnostic 配置
--   3. 注册 FileType autocmd 自动启用 LSP
--   4. 为已打开的 buffer 启用 LSP
-- 注意: 此文件不涉及插件增强 (如 blink.cmp, telescope 等)
...
-- 注册所有 LSP 服务器配置
-- Neovim 0.11+ 模式: 先 vim.lsp.config() 注册配置，再 vim.lsp.enable() 启用
local function register_lsp_configs()
    ...
    for _, server_name in ipairs(servers) do
        ...
        if vim.fn.filereadable(config_file) == 1 then
            -- 使用 loadfile + pcall 安全加载配置文件
            local load_func, load_err = loadfile(config_file)
            if load_func then
                local ok_conf, config = pcall(load_func)
...
-- 为已打开的 buffer 启用 LSP
-- 场景: 当 bootstrap.lua 加载时，可能已经有文件打开 (init.lua 中直接打开文件)
-- 问题: FileType autocmd 只处理新打开的文件，对已打开的 buffer 不会触发
-- 解决: 手动遍历所有已加载的 buffer，为其启用匹配的 LSP 服务器
...
-- 模块加载完成后立即为已打开的 buffer 启用 LSP
-- 为什么使用 vim.defer_fn(fn, 0):
--   1. 避免在模块加载过程中同步执行，保证所有模块初始化完成
--   2. 不使用 vim.schedule 以确保 LSP 尽早可用
--   3. FileType autocmd 已注册，会自动处理后续打开的文件
vim.defer_fn(function()
    enable_lsp_for_opened_buffers()
end, 0)
